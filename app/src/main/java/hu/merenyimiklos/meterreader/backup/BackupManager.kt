...
class BackupManager(
    ...
    fun import(uri: Uri): BackupPayload {
        ...
        val readingsArray =
            root.getJSONArray("readings")

        val readings = buildList {
            for (index in 0 until readingsArray.length()) {
                val item =
                    readingsArray.getJSONObject(index)

                add(
                    MeterReading(
                        id = item.getString("id"),
                        type = MeterType.valueOf(
                            item.getString("type")
                        ),
                        value = item.getDouble("value"),
                        dateEpochDay =
                            item.getLong("dateEpochDay"),
                        note =
                            item.optString("note", ""),
                        photoUri = null,
                        createdAtMillis =
    ...
    private fun settingsFromJson(
        ...
        gasBillingMode = runCatching {
            GasBillingMode.valueOf(
                item.optString(
                    "gasBillingMode",
                    GasBillingMode.FLAT_RATE.name
                )
            )
        }.getOrDefault(GasBillingMode.FLAT_RATE),
        gasUnitPrice =
            item.optDouble("gasUnitPrice", 0.0),
        gasMonthlyFixedFee =
            item.optDouble(
                "gasMonthlyFixedFee",
                0.0
